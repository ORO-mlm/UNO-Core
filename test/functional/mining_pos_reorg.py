...
class ReorgStakeTest(UnoTestFramework):
    ...
    def check_money_supply(self, expected_uno):
        # verify that nodes have the expected UNO supply
        uno_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(uno_supply, [DecimalAmt(expected_uno)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that UNO supply was properly updated after the reorgs
        self.log.info("Check UNO supply...")

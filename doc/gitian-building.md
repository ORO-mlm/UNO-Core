...
*Setup instructions for a gitian build of UNO Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the UNO
...
to the UNO GitHub release page.
...
- [Building UNO Core](#building-uno-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/uno-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/uno-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/uno-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/uno-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/uno-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/uno-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/uno-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@uno.cm) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/uno-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/uno-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/uno-osx-*-build.assert
```

...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/UNOParams"
    else
        PARAMS_DIR="$HOME/.uno-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
UNO - install-params.sh

This script will install the UNO zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common UNO zkSNARK parameters. Note that it is

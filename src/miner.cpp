...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("UNOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("UNOMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("UNOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("uno-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running UNOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("UNOMiner exception");
    } catch (...) {
        LogPrintf("UNOMiner exception");
    }

    LogPrintf("UNOMiner exiting\n");
}

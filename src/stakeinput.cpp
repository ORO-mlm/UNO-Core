...
CUnoStake* CUnoStake::NewUnoStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CUnoStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CUnoStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CUnoStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CUnoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CUnoStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CUnoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CUnoStake::GetUniqueness() const
{
    //The unique identifier for a UNO stake is the outpoint
...
const CBlockIndex* CUnoStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CUnoStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CUnoStake::ContextCheck(int nHeight, uint32_t nTime)
{

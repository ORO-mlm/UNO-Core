...
bool CLegacyZUnoStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZUnoStake from non zc-spend", __func__);
    ...
    *this = CLegacyZUnoStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zuno stake origin", __func__);

    // All good
    return true;
}

CLegacyZUnoStake::CLegacyZUnoStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZUnoStake::GetIndexFrom() const
{
...
CAmount CLegacyZUnoStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZUnoStake::GetUniqueness() const
{
...
bool CLegacyZUnoStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zUNO stake block: height %d outside range", __func__, nHeight);

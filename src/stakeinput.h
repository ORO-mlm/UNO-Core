...
#ifndef UNO_STAKEINPUT_H
#define UNO_STAKEINPUT_H
...
public:
    CUnoStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CUnoStake* NewUnoStake(const CTxIn& txin);
    ...
    bool IsZUNO() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //UNO_STAKEINPUT_H

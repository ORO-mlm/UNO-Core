...
const char * const UNO_CONF_FILENAME = "uno.conf";
const char * const UNO_PID_FILENAME = "uno.pid";
const char * const UNO_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\UNO
// Windows >= Vista: C:\Users\Username\AppData\Roaming\UNO
// Mac: ~/Library/Application Support/UNO
// Unix: ~/.uno
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UNO";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UNOParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UNOParams
    // Mac: ~/Library/Application Support/UNOParams
    // Unix: ~/.uno-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UNOParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "UNOParams";
#else
    // Unix
    return pathRet / ".uno-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/uno/sapling-spend.params";
        sapling_output = "/usr/share/uno/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/uno/sapling-spend.params";
            sapling_output = "/usr/local/share/uno/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", UNO_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", UNO_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty uno.conf if it does not exist
        ...
        if (configFile != NULL) {
            ...
            strHeader += "\naddnode=140.82.1.62:42001\naddnode=8.9.11.29:42001\naddnode=45.63.21.249:42001\naddnode=208.167.245.211:42001\naddnode=173.199.123.237:42001\naddnode=207.246.83.212:42001\naddnode=45.77.165.148:42001\n";
            strHeader += "\naddnode=explorer.uno.cm:42001\naddnode=ns1.uno.cm:42001\naddnode=ns2.uno.cm:42001\naddnode=ns3.uno.cm:42001\naddnode=ns4.uno.cm:42001\naddnode=ns5.uno.cm:42001\naddnode=ns6.uno.cm:42001\n";
            strHeader += "addnode=ns7.uno.cm:42001\naddnode=ns8.uno.cm:42001\naddnode=ns9.uno.cm:42001\naddnode=ns10.uno.cm:42001\naddnode=ns11.uno.cm:42001\naddnode=ns12.uno.cm:42001\n";
            strHeader += "addnode=ns13.uno.cm:42001\naddnode=ns14.uno.cm:42001\naddnode=ns15.uno.cm:42001\naddnode=ns16.uno.cm:42001\n";
            strHeader += "daemon=1\nserver=1\n";
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override uno.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", UNO_PID_FILENAME));

...
#include "warnings.h"
#include "zuno/zerocoin.h"
#include "zuno/zunomodule.h"
#include "zunochain.h"
...
#if defined(NDEBUG)
#error "UNO cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uuno) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k UNO
...
{
    util::ThreadRename("uno-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZUNOModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
                        return false;
                    }
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
        ...
        if (!masternodeSync.IsSynced()) {
            ...
            CAmount amtOut = 0;
            for (unsigned int i = 1; i < outs - 1; i++)
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Uno Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t)consensus.nUnoBadBlockTime) &&
            (block.nBits == (uint32_t)consensus.nUnoBadBlockBits)) {
            // accept UNO block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> unoInputs;
        std::vector<CTxIn> zUNOInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if (stakeIn.IsZerocoinSpend()) {
                zUNOInputs.push_back(stakeIn);
            } else {
                unoInputs.push_back(stakeIn);
            }
        }
        const bool hasUNOInputs = !unoInputs.empty();
        const bool hasZUNOInputs = !zUNOInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in : tx.vin) {
                if (consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZUNOModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if (hasUNOInputs) {
                    ...
                    for (const CTxIn& unoIn : unoInputs)
                        if (unoIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zUNO inputs.
            if (hasZUNOInputs) {
                for (const CTxIn& zUnoInput : zUNOInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zUnoInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zuno");
                }
            }
        }
            ...
            if (!isBlockFromFork)
                for (const CTxIn& zUnoInput : zUNOInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpend(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100, error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zuno");
                }
        }
    }

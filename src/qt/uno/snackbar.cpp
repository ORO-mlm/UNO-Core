...
#include "qt/uno/forms/ui_snackbar.h"
#include "qt/uno/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(UNOGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &UNOGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {

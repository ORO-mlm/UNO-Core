...
#include "qt/uno/send.h"
#include "qt/uno/forms/ui_send.h"
#include "qt/uno/addnewcontactdialog.h"
#include "qt/uno/qtutils.h"
#include "qt/uno/sendchangeaddressdialog.h"
#include "qt/uno/optionbutton.h"
#include "qt/uno/sendconfirmdialog.h"
#include "qt/uno/guitransactionsutils.h"
#include "qt/uno/loadingdialog.h"
...
SendWidget::SendWidget(UNOGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-uno");
    ...
    // Connect
    // connect(ui->pushLeft, &QPushButton::clicked, [this](){onUNOSelected(true);});
    // connect(ui->pushRight,  &QPushButton::clicked, [this](){onUNOSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent UNOs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield UNOs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent UNOs to shield."));
    }
}
...
void SendWidget::onUNOSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto unoAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(unoAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(unoAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(unoAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

...
#include "qt/uno/forms/ui_addresseswidget.h"
#include "qt/uno/addresslabelrow.h"
#include "qt/uno/addnewaddressdialog.h"
#include "qt/uno/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(UNOGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto unoAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(unoAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(unoAdd);
        if (walletModel->updateAddressBookLabels(unoAdd, label.toUtf8().constData(),

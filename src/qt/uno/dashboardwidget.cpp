...
#include "qt/uno/dashboardwidget.h"
#include "qt/uno/forms/ui_dashboardwidget.h"
#include "qt/uno/sendconfirmdialog.h"
#include "qt/uno/txrow.h"
...
DashboardWidget::DashboardWidget(UNOGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareUno, "square-chart-uno");
    // setCssProperty(ui->labelSquarezUno, "square-chart-zuno");
    setCssProperty(ui->labelUno, "text-chart-uno");
    // setCssProperty(ui->labelZuno, "text-chart-zuno");
    ...
    setCssProperty(ui->labelAmountUno, "text-stake-uno-disable");
    // setCssProperty(ui->labelAmountZuno, "text-stake-zuno-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &UNOGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("UNO")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isUno = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZUNO;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair UNO, zUNO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            uno = (pair.first != 0) ? pair.first / 100000000 : 0;
            zuno = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalUno += pair.first;
            chartData->totalZuno += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesUno.append(uno);
        chartData->valueszUno.append(zuno);

        int max = std::max(uno, zuno);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesUno);
    set1->append(chartData->valueszUno);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalUno > 0 || chartData->totalZuno > 0) {
        setCssProperty(ui->labelAmountUno, "text-stake-uno");
        // setCssProperty(ui->labelAmountZuno, "text-stake-zuno");
    } else {
        setCssProperty(ui->labelAmountUno, "text-stake-uno-disable");
        // setCssProperty(ui->labelAmountZuno, "text-stake-zuno-disable");
    }
    // forceUpdateStyle({ui->labelAmountUno, ui->labelAmountZuno});
    ui->labelAmountUno->setText(GUIUtil::formatBalance(chartData->totalUno, nDisplayUnit));
    // ui->labelAmountZuno->setText(GUIUtil::formatBalance(chartData->totalZuno, nDisplayUnit, true));

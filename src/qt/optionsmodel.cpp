...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::UNO);
    ...
    if (settings.contains("nAnonymizeUnoAmount") || reset)
        gArgs.SoftSetArg("-anonymizeunoamount", settings.value("nAnonymizeUnoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in uno.cpp during shotdown to also remove the window positions

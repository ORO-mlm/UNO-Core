...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select UNO Outputs to Spend" :
            "Select Shielded UNO to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::UNO;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show UNO address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {

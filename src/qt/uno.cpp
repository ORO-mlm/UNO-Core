...
#include "qt/uno/splash.h"
#include "qt/uno/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("uno-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in uno.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in uno.qrc)
...
/** Class encapsulating UNO Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (UNOGUI)
...
{
    window = new UNOGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &UNOGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &UNOGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(UNOGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(UNOGUI::DEFAULT_WALLET);
        ...
        // UNO: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &UNOGUI::handlePaymentRequest);
        connect(window, &UNOGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. UNO can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data directory and parse uno.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("UNO Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("UNO Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // uno: links repeatedly have their payment requests routed to this process:
